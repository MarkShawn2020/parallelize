...
export function TokenChart({ history, height }: Props) {
  ...
  const legend = (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted tabular-nums">
      <Key className="bg-s1" label="协调 Token Coordination" value={last ? fmtTokens(last.coordinationTokens) : undefined} />
      <Key className="bg-fg" label="正确 Correct" value={last ? `${last.correct}/${last.tasksTotal}` : undefined} />
      <Key className="bg-s2" label="升级率 Escalation" value={last ? fmtPct(last.escalationRate) : undefined} />
    </div>
  );
...
function Chart({ history, width, height }: { history: LiveMetrics[]; width: number; height: number }) {
  ...
  const yUnit = (v: number) => M.top + h - Math.min(1, Math.max(0, v)) * h;

  const coord = path(history.map((m) => [x(m.elapsedMs), yTok(m.coordinationTokens)]));
  const correct = path(history.map((m) => [x(m.elapsedMs), yUnit(m.tasksTotal > 0 ? m.correct / m.tasksTotal : 0)]));
  ...
  return (
    <svg width={width} height={height} className="absolute inset-0 font-mono" role="img" aria-label="协调 Token、正确数与升级率随时间变化">
      {ticks.map((f) => {
        const y = M.top + h - f * h;
        return (
          <g key={f}>
      ...
      <path d={escalation} fill="none" className="stroke-s2" strokeWidth={2} strokeDasharray="5 3" />
      <path d={correct} fill="none" className="stroke-fg" strokeWidth={2} />
      <path d={coord} fill="none" className="stroke-s1" strokeWidth={2.5} />
    </svg>
  );
}

function Key({ className, label, value }: { className: string; label: string; value: string | undefined }) {
  return (
