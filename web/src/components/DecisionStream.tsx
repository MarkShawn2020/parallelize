...
function describe(d: Decision): string {
  ...
  const precedents = d.precedentsUsed > 0 ? ` · 先例 precedents ${d.precedentsUsed}` : "";
  return `${d.key} · ${tier} · 置信 conf ${d.confidence.toFixed(2)} · ${fmtMs(d.latencyMs)}${precedents}${where ? ` · ${where}` : ""}`;
}
        ...
        <div className="flex h-12 items-end justify-end gap-px overflow-hidden" onMouseLeave={() => setHovered(null)}>
          ...
          {decisions.map((d) => (
            <div
              ...
              className={`w-1.5 shrink-0 ${d.tier === "system1" ? "bg-s1" : "bg-s2"} ${hovered?.id === d.id ? "outline outline-fg" : ""}`}
              style={{ height: `${Math.max(18, Math.round(d.confidence * 100))}%` }}
            />
          ))}
        </div>
        <div className="h-4 truncate text-xs text-muted tabular-nums">
          {hovered ? <span className="text-fg">{describe(hovered)}</span> : "悬停查看决策 hover a tick · 高度 = 置信度 height = confidence"}
        </div>
      </div>
    </Panel>
  );
