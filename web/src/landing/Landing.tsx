...
const REPO = "https://github.com/MarkShawn2020/parallelize";
...
const USAGE: ReadonlyArray<[string, string, string, boolean]> = [
  ...
  ["HTTP API", "启动一轮、实时操控、WebSocket 推送全部事件。", `curl -X POST localhost:8787/api/runs -d '{"mode":"swarm-jev","n":96}'`, true],
...
function Stat({ value, label, tone = "text-accent" }: { value: string; label: string; tone?: string }) {
  return (
    <div className="flex flex-col gap-1 border border-grid bg-panel/90 px-5 py-4">
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-40 border-b border-grid bg-bg/90 backdrop-blur">
        <nav className="mx-auto flex max-w-[1200px] items-center gap-6 px-4 py-3">
          <a href="#top" className="flex items-baseline gap-2 font-display text-2xl font-semibold tracking-[0.12em]">
            <span className="text-accent">JIS</span>
            <span className="hidden text-fg sm:inline">Jev in the Shell</span>
          </a>
          <div className="hidden flex-1 items-center gap-5 text-sm text-fg/75 lg:flex">
            {NAV.map(([href, label]) => (
          ...
          <a href={REPO} className="hidden text-sm text-fg/75 hover:text-fg sm:inline">
            GitHub
          </a>
          <a href={DEMO} className="border border-accent bg-accent/10 px-4 py-1.5 text-sm font-semibold text-accent hover:bg-accent hover:text-bg">
            {liveRunning ? "现场正在跑 →" : "进入现场演示 →"}
          </a>
        </nav>
      </header>
      ...
      <main id="top">
        <section className="relative overflow-hidden">
          <div className="mx-auto flex max-w-[1200px] flex-col gap-10 px-4 py-20 md:py-28">
            <div className="flex flex-col gap-5">
              <span className="font-mono text-sm tracking-widest text-accent">EvoTavern 4th · SECTION 9 多 Agent 蜂群协作 · 並列化 Parallelize</span>
            ...
            <div className="flex flex-wrap gap-3">
              <a href={DEMO} className="bg-accent px-6 py-3 text-lg font-semibold text-bg hover:bg-accent/85">
                进入现场演示
              </a>
              <a href="#paradigms" className="border border-fg/40 px-6 py-3 text-lg text-fg hover:border-fg">
                看九讲对比
              </a>
              <a href="#replay" className="border border-fg/40 px-6 py-3 text-lg text-fg hover:border-fg">
                看蜂群回放
              </a>
            </div>
          </div>
        ...
        >
          <div className="grid gap-3 md:grid-cols-3">
            <Stat value="96 道" label="困难档合成数学题：6–9 步推理，混入无关条件，至少一次单位、比率或百分比换算；算术 / 比率 / 逻辑各 32 道" tone="text-fg" />
